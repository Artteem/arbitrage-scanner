...
CHUNK = 200  # безопасный размер по числу подписок
...
async def run_bybit(store: TickerStore, symbols: Sequence[Symbol]):
    ...
    task = asyncio.create_task(_run_bybit_ws(store, subscribe))
...
async def _run_bybit_ws(store: TickerStore, subscribe: Sequence[Symbol]):
    if not subscribe:
        return

    ticker_topics = [f"tickers.{sym}" for sym in subscribe]
    orderbook_topics = [f"orderbook.50.{sym}" for sym in subscribe]
    topics = ticker_topics + orderbook_topics
    if not topics:
        return
        ...
        try:
            ...
            while True:
                ...
                try:
                    ...
                    logger.exception("Failed to decode Bybit payload", exc_info=True)
                    continue
            ...
            logger.warning("Bybit stream connection closed, reconnecting")
        except Exception:
            logger.exception("Bybit stream error", exc_info=True)
