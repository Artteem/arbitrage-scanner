...
"""
Modified BingX USDT‑perpetual connector.

This version aligns the BingX integration with the simplified connection logic
used by the Binance and Bybit connectors.  The key addition is a note
documenting the exchange's WebSocket subscription limit: according to a
support announcement, each BingX WebSocket may carry up to 200 topics
(subscriptions)【566159644440243†L178-L184】.  Because this connector
subscribes to two topics per contract (``ticker`` and ``depth5``), we limit
each connection to at most 100 contracts via the ``MAX_TOPICS_PER_CONN``
constant.  If more contracts are requested, multiple connections will be
spawned.

The remainder of this file corresponds to the upstream BingX connector.  It
implements discovery, symbol normalisation, subscription management and
message parsing for the exchange's USDT perpetual swap API.
"""
...
# topics (subscriptions) per WebSocket【566159644440243†L178-L184】.  Each contract
# results in two subscriptions (ticker and depth), so we group at most 100
# contracts per connection.
